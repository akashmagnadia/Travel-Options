...
public:
	...
	*/
	bool is_sorted() const { /*
		// error orginating with the following in main:
		//options_p3 = options_p->union_pareto_sorted(*options_p2);
		//making options_p3 null therefore making front null as well
		Node *p = front;

		for (int i = 0; i < size() - 1; i++) {
			if (p->price > p->next->price) { //if the next price is higher than the current price
				return false;
			}
			else {
				if (p->price == p->next->price) { //if the prices are the same AND
					if (p->time > p->next->time) { //if the next time is less
						return false;
					}
				}
			}
	...
	*/
	bool is_pareto() const {
		Node *p = front;
		Node *temp = front; //another linked list
		double difference = size(); //stores difference between two values
		double currentPrice; //to keep track of price that is being checked
		double currentTime; //to keep track of time that is being checked
		double heldPrice = p->price; //price that is going to be checked with current price
		double heldTime = p->time; //time that is going to be checked with current time
		bool originalhit;

		//p = p->next; temporary disabled
		//since first one is not being compared to anything
		// and for loop is also decresed by 2 for that reason 
		//1)skipping first 2)comparing ->next
		for (int i = 0; i < size() - 1; i++) {
			currentPrice = p->price;
			currentTime = p->time;
			originalhit = false;

			temp = front; //reset the linked list to front

			for (int j = 0; j < size() - 1; j++) {
				if (currentPrice == temp->price) {
				...
				if (currentPrice - temp->price == difference) {
					if (heldTime > temp->time) {
						return false;
					}
				}

				if ((currentPrice - temp->price > 0) && (currentPrice - temp->price < difference)) {
					if (heldTime > temp->time) {
						return false;
					}
					difference = currentPrice - temp->price;
					heldPrice = temp->price;
					heldTime = temp->time;
				}
				temp = temp->next;
			}
	...
	*/
	bool is_pareto_sorted() const {
		...
		for (int i = 0; i < size() - 1; i++) {
			if ((p->price > p->next->price) || (p->time < p->next->time)) {
				//if the next price is lower and time is higher than the current price
				return false;
			}

			//if the next price is higher and time is lower than the current price
			p = p->next; //move to the next block
		}
	...
	bool insert_sorted(double price, double time) {
		...
		Node *p = front;
		Node *temp = new Node; //make a temp node to add to the end
		temp->price = price;
		temp->time = time;
		temp->next = nullptr;

		for (int i = 0; i < size()-1; i++) {
			if (i == 0) { //comparing to the first value
				if (p->price > price) {//if the first value is more than given price
				temp->next = front;
				front = temp;
				return true;
				}

				if (p->price == price) { //if the first price is same as the given price
					if (time < p->time) { //if the first time is more than the given time
						temp->next = front;
						front = temp;
						return true;
					}
					else { //if the first time is less than the given time
			...
			if (price > p->price) { //if the price is less than the given price
				if (price < p->next->price) { //if the next price is more than the given price
					temp->next = p->next;
					p->next = temp;
					return true;
				}
			...
			if (price == p->next->price) { //if the price is the same as given price
				if (time > p->next->time) { //if the given time is more than the next time
					if (i < size()-2) {
						if (p->next->next->price == price) {
							p = p->next;
							continue;
						}
						else {
				...
				else { //if the given time is less than the next time or both times are equal
					temp->next = p->next;
					p->next = temp;
					return true;
				}
	...
	bool insert_pareto_sorted(double price, double time) {
		...
		for (int i = 0; i < size() - 1; i++) {
			if (i == 0) { //checking at the first option
				if ((p->price > price) && (p->time < time)) { //if the price is lower and time is higher of the given value
					temp->next = front;
					front = temp;
					return true;
				}
			...
			if (price > p->price) { //if the current price is less than the given price
				if (price < p->next->price) { //if the next price is more than the given price
					if (p->time == time) { //if the current time is same as given time
						return true;
					} else {
						p->next->price = price;
						return true;
			...
			if (price == p->price) { //if the current value has the same price
				if (time < p->time) { //if the given time is less than the current time
					p->time = time;
					return true;
				}
				else {
					return true; //keep the list unchanged since the inserted value will be dominated
				}
			}

			if (i == size() - 1) { //if nothing else plugs in put it at the end
				p->next = temp; 
			}
